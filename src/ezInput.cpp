...
void ezInput::init(ezWidget* pwPtr,
  ...
  text    = text_;
  prompt  = prompt_;
  colors  = ez.Theme.colors(colors_, ez.Theme.inp_colors);
  font    = (font_ != THEME_FONT) ? font_ : ez.Theme.inp_font;
  align   = align_;
  valign  = valign_;
  padding = padding_;
  dx      = dx_;
...
String keyboardInput(String prompt_ /* = "" */, String text_ /* = "" */) {
  ...
  kb->keys.colors = ez.Theme.kb;
  for (int8_t n = 0; n < MAX_KEYS; n++) {
    kb->k[n].label.font   = ez.Theme.kb_keyFont;
    kb->k[n].colors       = ez.Theme.kb_keys;
    kb->k[n].onColors     = ez.Theme.kb_keysOn;
    kb->k[n].dbltapTime   = 0;
    kb->k[n].userData     = 1;
    kb->keys.add(kb->k[n]);
  }
